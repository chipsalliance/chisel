...
case class Axi4IOParameters(
  ...
  require(idBits >= 0, s"AXI4 id bits must be non-negative (got $idBits)")
...
object Axi4Response extends ChiselEnum {
...
class Axi4AddressChannel(params: Axi4IOParameters, userBits: Int = 0) extends Bundle {
  ...
  val qos = UInt(4.W)
  private val regionWidth = if (params.enableRegionBits) { 4 }
  else { 0 }
