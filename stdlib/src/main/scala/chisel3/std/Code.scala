...
class ParityCode extends Code
{
  ...
  def encode(x: UInt, poison: Bool = false.B) = x.xorR ^ poison ## x
  def swizzle(x: UInt) = false.B ## x
  def decode(y: UInt) = new Decoding {
...
class SECCode extends Code
{
  ...
  def encode(x: UInt, poison: Bool = false.B) = {
    ...
    val syndromeUInt = VecInit(Seq.tabulate(n-k) { j => (syndrome(j)(k-1, 0) & x).xorR ^ poison }).asUInt
    syndromeUInt ## x
  }
  ...
  def decode(y: UInt) = new Decoding {
    ...
    val syndromeUInt = VecInit(Seq.tabulate(n-k) { j => (syndrome(j) & y).xorR }).asUInt

    val hammBadBitOH = UIntToOH(syndromeUInt, n+1)
    val sysBadBitOH = VecInit(Seq.tabulate(k) { i => hammBadBitOH(sys2hamm(i)) }).asUInt
...
class SECDEDCode extends Code
{
  ...
  def encode(x: UInt, poison: Bool = false.B): UInt = {
    ...
    val toggle_lo = poison.asUInt ## poison.asUInt
...
object ErrGen
{
  ...
  def apply(width: Int, f: Int): UInt = {
    require(width > 0 && f >= 0 && log2Ceil(width) + f <= 16)
