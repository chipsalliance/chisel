...
sealed class Vec[T <: Data] private (gen: T, val length: Int)
  ...
  */
  ...
  def <> (that: Vec[T]): Unit = this bulkConnect that.asInstanceOf[Data]
...
trait VecLike[T <: Data] extends collection.IndexedSeq[T] {
  ...
  /** Outputs true if p outputs true for every element.
    */
  def forall(p: T => Bool): Bool = (this map p).fold(true.asBool)(_ && _)

  /** Outputs true if p outputs true for at least one element.
    */
  def exists(p: T => Bool): Bool = (this map p).fold(false.asBool)(_ || _)
  ...
  /** Helper function that appends an index (literal value) to each element,
    ...
    */
  private def indexWhereHelper(p: T => Bool) = (this map p) zip ((0 until length).map(_.asUInt))
