...
object Reg {
  ...
  /** Creates a register without initialization (keeps its previous value on
    ...
    * @param t: data type for the register
    */
  def apply[T <: Data](t: T): T = Reg[T](t, null.asInstanceOf[T], null.asInstanceOf[T])
}

object Mem {
  @deprecated("Chisel3 Mem argument order should be size, t - this will be removed by Chisel3 official release", "now")
  def apply[T <: Data](t: T, length: Int): Mem[T] = apply(length, t)

  /** Creates a combinational-read, synchronous-write [[Mem]].
    * 
    * @param length number of elements in the memory 
    * @param t data type of memory element 
    */
  def apply[T <: Data](length: Int, t: T): Mem[T] = {
    val mt  = t.cloneType
    val mem = new Mem(mt, length)
    pushCommand(DefMemory(mem, mt, length, Node(mt._parent.get.clock))) // TODO multi-clock
    mem
  }
}
...
/** A combinational-read, synchronous-write memory. 
  ...
  * "continuously" (read requests will return data on the same cycle).
...
object SeqMem {
  ...
  def apply[T <: Data](t: T, length: Int): SeqMem[T] = apply(length, t)
  ...
  /** Creates a synchronous-read, synchronous-write [[SeqMem]].
    ...
    * @param length number of elements in the memory 
    */
  def apply[T <: Data](length: Int, t: T): SeqMem[T] = {
    val mt  = t.cloneType
    val mem = new SeqMem(mt, length)
    pushCommand(DefSeqMemory(mem, mt, length, Node(mt._parent.get.clock))) // TODO multi-clock
    mem
  }
}
...
/** A synchronous-read, synchronous-write memory.
  ...
  * undefined.
  */
sealed class SeqMem[T <: Data](t: T, length: Int) extends MemBase[T](t, length) {
...
object Vec {
  ...
  /** Creates a new [[Vec]] composed of elements of the input Seq of [[Data]]
    * nodes.
    * 
    * @note input elements should be of similar width and type
    * @note output elements are connected from the input elements
    */
  def apply[T <: Data](elts: Seq[T]): Vec[T] = {
    // REVIEW TODO: error checking to guard against length mismatch?
...
trait VecLike[T <: Data] extends collection.IndexedSeq[T] {
  ...
  // TODO: find a way to make images link properly.
  /** Outputs true if p outputs true for every element.
    ...
    * 
    * @note Block diagram:
    * <img src="../../../../src/main/scala/Chisel/doc/vec-forall.svg" />
    */
...
object BitPat {
  ...
  private def parse(x: String): (BigInt, BigInt, Int) = {
    ...
    for (d <- x.tail) {
      if (d != '_') {
        if (!"01?".contains(d)) Builder.error({ s"BitPat literal '$x' contains illegal character '$d'" })
...
sealed abstract class Bits(dirArg: Direction, width: Width, override val litArg: Option[LitArg]) extends Element(dirArg, width) {
  ...
  // REVIEW TODO: Why do we need a fromInt?
  ...
  /** Returns the specified bit on this wire as a [[Bool]], statically
    ...
    */
  // REVIEW TODO: Do we really need a BigInt here? Will we *really* have wires
  // that are > ~2^32 long?
  ...
  final def apply(x: Int, y: Int): UInt = {
    if (x < y) {
      Builder.error(s"Invalid bit extraction: hi $x must be larger than lo $y")
    } else if (y < 0) {
      Builder.error(s"Invalid bit extraction: cannot have negative index")
      // REVIEW TODO: can we support negative indexing Python style, at least
  ...
  // REVIEW TODO: ... REALLY?!?! ... REALLY?!?! this should FAIL the smell test!
...
/** Abstract trait defining operations available on numeric-like wire data 
  * types.
  * 
  * Perhaps operations can also be defined here, using this as a base trait for
  * both SInt and UInt to simplify and deduplicate logic?
  */
