...
/** This is the annotation created when using [[loadMemoryFromFileBind]], it records the memory, the load file
...
/** [[loadMemoryFromFileBind]] is an annotation generator that helps with loading a memory from a text file as a bind module. This relies on
  ...
  * {{{
  * import chisel3._
  * import chisel3.util.experimental.loadMemoryFromFileBind   // <<-- new import here
  * class UsesMem(memoryDepth: Int, memoryType: Data) extends Module {
  *   val io = IO(new Bundle {
  *     val address = Input(UInt(memoryType.getWidth.W))
  *     val value   = Output(memoryType)
  ...
  *   val memory = Mem(memoryDepth, memoryType)
  *   io.value := memory(io.address)
  *   loadMemoryFromFileBind(memory, "/workspace/workdir/mem1.hex.txt")  // <<-- Note the annotation here
  * }
  * }}}
  *
  * ==Example file format==
  ...
  *
  * @see
  * [[https://github.com/freechipsproject/chisel3/tree/master/src/test/scala/chiselTests/LoadMemoryFromFileBindSpec.scala
  * LoadMemoryFromFileBindSpec.scala]] in the test suite for additional examples.
