...
sealed trait FixedIOBaseModule[A] extends BaseModule {
  ...
  final val io: A = {
    implicit val l: Lookupable[A] = lookupable
    val dataElems = l.in(ioGenerator)
    ...
    l.out(ioGenerator, ports.iterator)
  }
  endIOCreation()
...
/** A Chisel module whose IO is determined by an IO generator.  This module
  ...
  * @param ioGenerator
  */
class FixedIORawModule[A](final val ioGenerator: A)(implicit val lookupable: Lookupable[A])
    extends RawModule
 ...
 *  extend it.
  ...
  * @param ioGenerator
  */
class FixedIOModule[A](final val ioGenerator: A)(implicit val lookupable: Lookupable[A])
    extends Module
    with FixedIOBaseModule[A]
