    ...
    def getNameIndex(name: String)(implicit sourceInfo: SourceInfo): Int = {
      ...
      if (isLit(name)) buildLitIndex(name) // Build index if it is a literal, otherwise error because something is wrong
        ...
        names.get(name) match {
          case Some(i) => i
          case None =>
            throwInternalError(sourceInfo.makeMessage(x => s"$x: Name \"$name\" not found when building dedup hash for ${hier.name} (${hier.proto._getSourceLocator.makeMessage(x => x)}):\n${hier.chirrtlString}"))
        }
    }
