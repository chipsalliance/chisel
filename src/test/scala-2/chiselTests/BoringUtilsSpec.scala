...
class BoringUtilsSpec extends ChiselFlatSpec with ChiselRunners with Utils with FileCheck with ChiselSim {
  ...
  it should "connect across modules using BoringUtils.bore" in {
    simulate(new TopTester)(Stimulus.runUntilFinished(3))
  }
  ...
  it should "still work even with dedup off" in {
    simulate(new TopTesterFail)(Stimulus.runUntilFinished(3))
  }
  ...
  it should "work for an internal, same module, BoringUtils.bore" in {
    simulate(new InternalBoreTester)(Stimulus.runUntilFinished(3))
  }
