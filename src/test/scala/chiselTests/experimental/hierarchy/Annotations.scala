...
private[hierarchy] object Annotations {
  ...
  def mark(d:                  HasTarget, tag:    String): Unit = annotate()(Seq(MarkAnnotation(d.toTarget, tag)))
  def mark[B <: BaseModule](d: Hierarchy[B], tag: String): Unit = annotate()(Seq(MarkAnnotation(d.toTarget, tag)))
  def amark(d: Data, tag: String): Unit = annotate(d)(Seq(MarkAnnotation(d.toAbsoluteTarget, tag)))
  def amark[B <: BaseModule](d: Hierarchy[B], tag: String): Unit = annotate()(Seq(MarkAnnotation(d.toTarget, tag)))
}
