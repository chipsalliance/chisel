...
// A pointer to global state that all functions will reference.  This is owned
// by the `main` function and is only valid while `main` is in scope, i.e., for
...
static void writeMessageStart(char messageCode) {
  if (shouldLogMessageToExecutionScript()) {
    fprintf(state->executionScript, "%d< %c ",
            state->executionScriptMessageCount, messageCode);
  }
...
int main(int argc, const char *argv[]) {
  ...
  // Stack allocate global mutable state and set the global state pointer to
  // this.  Every function in this file relies on access to this.  It would be
  // _better_ if there was an alternative approach that avoided the global
  // mutable state.
  State _state;
  state = &_state;
