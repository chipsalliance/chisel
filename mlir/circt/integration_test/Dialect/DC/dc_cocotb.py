...
def struct_type(fieldTypes):
  ...
  # This places assumptions on the struct naming standard of DCToHW,
...
class DCPort:
  ...
  async def awaitDC(self):
    ...
    if (directSend):
      # If it was initially ready, the DC happens in the current cycle.
...
@dataclass
class DCDataPort(DCPort):
  """
  A !dc.value<...>-typed port.
  """
  data: object
...
def _findPort(dut, name):
  ...
  returns a DCPort that encapsulates the dut's interface.
  """
  ...
  if not isinstance(data, type(None)):
    return DCDataPort(dut, ready, valid, data)

  isCtrl = not hasattr(dut, f"{name}_data_field0")

  if (isCtrl):
    return DCPort(dut, ready, valid)
