...
private[chisel3] object Serializer {
  ...
  private def serialize(arg: Arg, ctx: Component, info: SourceInfo)(implicit b: StringBuilder): Unit = arg match {
    ...
    case Slot(imm, name) => serialize(imm, ctx, info); b += '.'; b ++= name
  ...
  // /** Serialize Commands */
  // TODO can we remove the Iterator boxing?
  // And manual indention
  def serializeSimpleCommand(cmd: Command, ctx: Component, typeAliases: Seq[String])(
  ...
  def serializeCommand(cmd: Command, ctx: Component, typeAliases: Seq[String])(implicit indent: Int): Iterator[String] =
    cmd match {
      case When(info, pred, ifRegion, elseRegion) =>
        ...
        val middle = ifRegion.flatMap(serializeCommand(_, ctx, typeAliases)(indent + 1))
  ...
  def serialize(name: String, param: Param)(implicit b: StringBuilder): Unit = param match {
  ...
  def serialize(param: TestParam)(implicit b: StringBuilder, indent: Int): Unit = param match {
  ...
  def serialize(component: Component, typeAliases: Seq[String])(implicit indent: Int): Iterator[String] =
    component match {
