...
private[chisel3] object binding {
  ...
  sealed trait ViewWriteability {
    ...
    /** Report errors if this is a read-only view
      ...
      * Will use onFail value to recover if this is an error
      */
  ...
  /** Binding for marking a signal as "soon-to-be deprecated" so that warnings can be printed
    *
    * @todo TODO, a bidirectional aware version of this would be more general, "source" rather than "read only"
    */
  case class DeprecatedReadOnlyViewBinding(target: Data, getWarning: SourceInfo => Warning)
      extends Binding
      with ConditionalDeclarable {
    def location: Option[BaseModule] = target.binding.flatMap(_.location)
    def visibility: Option[WhenContext] = target.binding.flatMap {
