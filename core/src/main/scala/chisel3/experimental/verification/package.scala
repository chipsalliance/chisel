...
package object verification {
  ...
  object require {
    def apply(
      ...
      pushCommand(Verification(Formal.Require, sourceInfo, clock.ref, predicate.ref, msg))
    }
  }
  ...
  object ensure {
    def apply(
      ...
      val clock = Builder.forcedClock
      pushCommand(Verification(Formal.Ensure, sourceInfo, clock.ref, predicate.ref, msg))
    }
  }
}
