...
private[chisel3] object Utils {
  ...
  //  module:  Proxy[T],
  //  context: BaseModule // TODO! This needs to be Hierarchy, so we can get the contexts and pass to ModuleMock, so they are accessible in the Select operators.
  //)(
  //  implicit sourceInfo: SourceInfo,
  //  compileOptions:      CompileOptions
  //): Proxy[T] = {
  ...
  //        case ic: IsStandIn[ _] => Module.do_pseudo_apply(new ModuleMock(x, name, p))
  //        case other => Module.do_pseudo_apply(new ModuleMock(other, name, p))
  //      }
  //      StandIn(newChild)
  //    }
  //    (m, context) match {
  ...
  //    case StandIn(m: experimental.hierarchy.ModuleClone[T]) =>
  //      //println("experimental.hierarchy.ModuleClone")
  //      rec(m) match {
  //        case Proto(mx, _) => StandIn(mx)
  //        case StandIn(i: ModuleMock[T]) =>
  //          val newChild = Module.do_pseudo_apply(new ModuleMock(m.proto, () => m.instanceName, i._parent))
  //          StandIn(newChild)
  //      }
  //    case StandIn(m: ModuleMock[_]) =>
  //      //println("ModuleMock")
  //      rec(m) match {
  //        case Proto(mx, _) => StandIn(mx)
  //        case StandIn(i: ModuleMock[_]) =>
  //          val newChild = Module.do_pseudo_apply(new ModuleMock(m.proto, () => m.instanceName, i.parent))
  //          StandIn(newChild)
  //      }
  //  }
  //}
