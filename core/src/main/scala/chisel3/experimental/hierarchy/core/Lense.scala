...
//
//  private[chisel3] val cache = mutable.HashMap[Any, Any]()
//
...
//    retValue
//  }
//  def toHierarchy: Hierarchy[P] = { new Instance(proxy) }
//}
//
//final case class Socket[T](contextual: Contextual[T], lense: Lense[_]) {
//  def value: T = contextual.value
//  def value_=(newValue: T): Unit = edit({_: T => newValue})
//  def edit(f: T => T): Edit[T] = lense.proxy match {
//    case StandIn(s: IsContext) => s.addEdit(contextual, f)
...
//      Lense(lookuper(value, lense.toHierarchy).asInstanceOf[Instance[I]].proxy)
//    }
//  }
