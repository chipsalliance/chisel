...
trait Lookuper[-V]  {
  type R
  def apply[P](value: V, proxy: Proxy[P]): R
}
...
object Lookuper {
  ...
  implicit val lookuperInt = new Lookuper[Int] {
    type R = Int
    def apply[P](that: Int, proxy: Proxy[P]): R = that
  }
  implicit val lookuperString = new Lookuper[String] {
    type R = String
    def apply[P](that: String, proxy: Proxy[P]): R = that
  }
  implicit def lookuperOption[B](implicit lookuper: Lookuper[B]) = new Lookuper[Option[B]] {
    type R = Option[lookuper.R]
    def apply[P](that: Option[B], proxy: Proxy[P]): R = {
      that.map { x: B => lookuper[P](x, proxy) }
    }
  }
  implicit def lookuperEither[X, Y](implicit lookuperX: Lookuper[X], lookuperY: Lookuper[Y]) = new Lookuper[Either[X, Y]] {
    ...
    def apply[P](that: Either[X, Y], proxy: Proxy[P]): R = {
      that.map { y: Y => lookuperY[P](y, proxy) }
          .left
          .map { x: X => lookuperX[P](x, proxy) }
    }
  }
  implicit def lookuperTuple2[X, Y](implicit lookuperX: Lookuper[X], lookuperY: Lookuper[Y]) = new Lookuper[Tuple2[X, Y]] {
    ...
    def apply[P](that: Tuple2[X, Y], proxy: Proxy[P]): R = {
      (lookuperX[P](that._1, proxy), lookuperY(that._2, proxy))
    }
  }
  ...
  //) = new Lookuper[F[B]] {
  //  type R = F[lookuper.R]
  //  def apply[P](that: F[B], proxy: Proxy[P]): R = {
  //    val ret = that.asInstanceOf[Iterable[B]]
  //    ret.map { x: B => lookuper[P](x, proxy) }.asInstanceOf[R]
  //  }
  //}
  implicit def isLookupable[V <: IsLookupable] = new Lookuper[V] {
    type R = V
    def apply[P](value: V, proxy: Proxy[P]): R = value
  }

  //implicit def lookuperContextual[V] = new Lookuper[Contextual[V]] {
  //  type R = V
  //  def apply[P](v: Contextual[V], proxy: Proxy[P]): V = {
  //    proxy match {
  //      case i: Instance[P] => i.open(v)
  //      case other => v.value
  //    }
  //  }
  //}
