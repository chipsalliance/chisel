...
abstract class Data extends HasId with NamedComponent with SourceInfoDoc {
  ...
  /** Bi-directional connection from a producer to consumer.
    *
    * When used as `consumer :<>= producer`, the following algorithm applies:
    *
    *  - Leaf fields aligned in relative direction to consumer will be assigned to from the corresponding producer field
    *  - Leaf fields flipped in relative direction to producer will be assigned to from the corresponding consumer field
    *  - The Chisel type of this and that must be equivalent:
