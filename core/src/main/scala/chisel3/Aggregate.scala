...
abstract class Record extends Aggregate {
  ...
  // Compiler-generated list of parameter accessors. Used to help generate a typeName for Bundle types
  // using parameter information directly defined in its constructor
  protected def _typeNameConParams: Iterable[Any] = Map.empty
...
abstract class Bundle extends Record {
  ...
  /** The typeName for Bundles, which is when desired auto generated by using the provided bundle arguments.
    * @note
    */
  override def typeName: String =
    if (_typeNameConParams.nonEmpty) {
      autoTypeName(getClass.getSimpleName, _typeNameConParams)
    } else {
      super.typeName
    }

  private def autoTypeName(bundleName: String, typeNameParams: Iterable[Any]): String =
    _typeNameConParams.foldLeft(getClass.getSimpleName) {
