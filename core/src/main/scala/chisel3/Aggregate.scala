...
private[chisel3] trait VecVirtualMethods[T <: Data] {
...
sealed class Vec[T <: Data] private[chisel3] (gen: => T, val length: Int)
  ...
  override private[chisel3] def _applyImpl(p: UInt)(implicit sourceInfo: SourceInfo): T = {
  ...
  override private[chisel3] def _reduceTreeImpl(
    redOp:   (T, T) => T,
    layerOp: (T) => T = (x: T) => x
  )(
    implicit sourceInfo: SourceInfo
...
private[chisel3] trait VecInit$VirtualMethods {
...
object VecInit extends VecInit$Intf {
  ...
  override private[chisel3] def _applyImpl[T <: Data](elts: Seq[T])(implicit sourceInfo: SourceInfo): Vec[T] = {
  ...
  override private[chisel3] def _applyImpl[T <: Data](elt0: T, elts: T*)(implicit sourceInfo: SourceInfo): Vec[T] =
...
private[chisel3] trait VecLikeVirtualMethods[T <: Data] {
...
trait VecLike[T <: Data] extends VecLikeImpl[T] with IndexedSeq[T] with HasId {
  ...
  override private[chisel3] def _forallImpl(p: T => Bool)(implicit sourceInfo: SourceInfo): Bool =
    (this.map(p)).fold(true.B)(_ && _)

  override private[chisel3] def _existsImpl(p: T => Bool)(implicit sourceInfo: SourceInfo): Bool =
    (this.map(p)).fold(false.B)(_ || _)

  override private[chisel3] def _containsImpl(x: T)(implicit sourceInfo: SourceInfo, ev: T <:< UInt): Bool =
    this._existsImpl(_ === x)

  override private[chisel3] def _countImpl(p: T => Bool)(implicit sourceInfo: SourceInfo): UInt =
  ...
  override private[chisel3] def _indexWhereImpl(p: T => Bool)(implicit sourceInfo: SourceInfo): UInt =
    SeqUtils.priorityMux(indexWhereHelper(p))

  override private[chisel3] def _lastIndexWhereImpl(p: T => Bool)(implicit sourceInfo: SourceInfo): UInt =
    SeqUtils.priorityMux(indexWhereHelper(p).reverse)

  override private[chisel3] def _onlyIndexWhereImpl(p: T => Bool)(implicit sourceInfo: SourceInfo): UInt =
