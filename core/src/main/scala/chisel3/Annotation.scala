...
object doNotDedup {
  ...
  def apply[T <: RawModule](module: T): Unit = {
    annotate(new ChiselAnnotation { def toFirrtl: Annotation = NoDedupAnnotation(module.toNamed) })
  }
}
...
object dedupGroup {
  ...
  def apply[T <: BaseModule](module: T, group: String): Unit = {
    annotate(new ChiselAnnotation { def toFirrtl: Annotation = DedupGroupAnnotation(module.toTarget, group) })
  }
}
