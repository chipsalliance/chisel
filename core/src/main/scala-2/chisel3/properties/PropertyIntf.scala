...
private[chisel3] trait Property$Intf[T] { self: Property[T] =>
  ...
  final def +(
    that: Property[T]
  )(implicit ev: PropertyArithmeticOps[Property[T]], sourceInfo: SourceInfo): Property[T] =
    _addImpl(that)
  ...
  final def ++(
    that: Property[T]
  )(implicit ev: PropertySequenceOps[Property[T]], sourceInfo: SourceInfo): Property[T] =
    _concatImpl(that)
}
