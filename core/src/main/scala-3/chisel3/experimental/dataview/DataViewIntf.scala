...
private[chisel3] trait DataView$ObjIntf { self: DataView.type =>
  ...
  def apply[T: DataProduct, V <: Data](
    mkView: T => V,
    pairs:  ((T, V) => (Data, Data))*
  )(using sourceInfo: SourceInfo): DataView[T, V] = _applyImpl(mkView, pairs: _*)
  ...
  def pairs[T: DataProduct, V <: Data](
    mkView: T => V,
    pairs:  ((T, V) => (Data, Data))*
  )(using sourceInfo: SourceInfo): DataView[T, V] = _pairsImpl(mkView, pairs: _*)

  /** More general factory method for complex mappings */
  def mapping[T: DataProduct, V <: Data](
    mkView:  T => V,
    mapping: (T, V) => Iterable[(Data, Data)]
  )(using sourceInfo: SourceInfo): DataView[T, V] = _mappingImpl(mkView, mapping)
...
private[chisel3] trait DataViewableIntf[T] { self: DataViewable[T] =>
  def viewAs[V <: Data](
    using dataproduct: DataProduct[T],
...
private[chisel3] trait RecordUpcastableIntf[T <: Record] { self: RecordUpcastable[T] =>
  ...
  def viewAsSupertype[V <: Record](proto: V)(using ev: ChiselSubtypeOf[T, V], sourceInfo: SourceInfo): V =
