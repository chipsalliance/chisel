...
private[chisel3] trait SimLogIntf { self: SimLog =>
  ...
  /** Prints a message in simulation
    ...
    * ==Format Strings==
    *
    * This method expects a ''format string'' and an ''argument list''
    * in a similar style to printf in C. The format string expects a
    * [[scala.Predef.String String]] that may contain ''format
    * specifiers'' For example:
    * {{{
    *   printf("myWire has the value %d\n", myWire)
    * }}}
    * This prints the string "myWire has the value " followed by
    * the current value of `myWire` (in decimal, followed by a
    * newline.
    *
    ...
    * ===Format Specifiers===
    *
    * Format specifiers are prefixed by `%`. If you wish to print a
    * literal `%`, use `%%`.
    *   - `%d` - Decimal
    *   - `%x` - Hexadecimal
    *   - `%b` - Binary
